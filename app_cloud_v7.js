...
function verifyLicenseGuard() {
  ...
  const storedLastRun = safeLocalStorage.getItem(lastRunKey);
  ...
  if (lastRun > 0 && now < (lastRun - 86400000)) { // Clock backdated by > 24 hours
    ...
    if (now > lastRun) {
      safeLocalStorage.setItem(lastRunKey, now.toString());
    }
  }
...
function findIndividualColKey(item) {
  if (!state.individual || state.individual.length < 4) return "E";
  ...
  // Combine RP-3.82 (Current Year) & RP-3.83 (Previous Year) under Column E "Subscription (Min 200.00)"
  ...
  return "E"; // Default to Subscription Column E
}
...
function commitCartToLedgers() {
  ...
  safeLocalStorage.setItem("CHURCH_CASHBOOK", JSON.stringify(state.cashbook));
  safeLocalStorage.setItem("CHURCH_MEMBERS", JSON.stringify(state.individual));
  safeLocalStorage.setItem("CHURCH_RECEIPT_NO", state.currentReceiptNo.toString());
  safeLocalStorage.setItem("CHURCH_VOUCHER_NO", state.currentVoucherNo.toString());
...
function saveReceiptPrintCopy(docNo, prefix, htmlContent, callback) {
  ...
  try {
    const savedMap = JSON.parse(safeLocalStorage.getItem("CHURCH_SAVED_RECEIPTS") || "{}");
    savedMap[key] = htmlContent;
    savedMap[`${prefix}_#${cleanDocNo}`] = htmlContent;
    safeLocalStorage.setItem("CHURCH_SAVED_RECEIPTS", JSON.stringify(savedMap));
  } catch (e) { }
...
function saveNewAccountHead() {
  ...
  if (Array.isArray(state.deletedAccountHeads)) {
    ...
    safeLocalStorage.setItem("CHURCH_DELETED_HEADS", JSON.stringify(state.deletedAccountHeads));
  }
  ...
  safeLocalStorage.setItem("CHURCH_ACCOUNT_HEADS", JSON.stringify(state.customAccountHeads));
...
function updateAdminPassword() {
  ...
  safeLocalStorage.setItem("CHURCH_ADMIN_PASS", newPass);
...
function resetSystemDataOverrides() {
  if (confirm("Are you sure you want to reset all custom local data changes (members, account heads, edited transactions) and revert to default Excel workbook datasets?")) {
    safeLocalStorage.removeItem("CHURCH_MEMBERS");
    safeLocalStorage.removeItem("CHURCH_ACCOUNT_HEADS");
    safeLocalStorage.removeItem("CHURCH_DELETED_HEADS");
    safeLocalStorage.removeItem("CHURCH_DELETED_MEMBERS");
    safeLocalStorage.removeItem("CHURCH_CASHBOOK");
    safeLocalStorage.removeItem("CHURCH_ADMIN_PASS");
    location.reload();
  }
}
  ...
  try {
    ...
    if (confirm(`Are you sure you want to restore backup from ${data.exportDate || 'selected backup'}? This will update cashbook entries and member records.`)) {
      ...
      if (Array.isArray(data.trialBalance)) {
        ...
        safeLocalStorage.setItem("CHURCH_TRIAL_BALANCE", JSON.stringify(state.trialBalance));
      }
      if (Array.isArray(data.codes)) {
        state.codes = data.codes;
        safeLocalStorage.setItem("CHURCH_CODES", JSON.stringify(state.codes));
      }
      if (Array.isArray(data.budget)) {
        state.budget = data.budget;
        safeLocalStorage.setItem("CHURCH_BUDGET", JSON.stringify(state.budget));
      }
      if (Array.isArray(data.auction)) {
        state.auction = data.auction;
        safeLocalStorage.setItem("CHURCH_AUCTION", JSON.stringify(state.auction));
      }
      if (Array.isArray(data.members)) {
        state.members = data.members;
        safeLocalStorage.setItem("CHURCH_MASTER_MEMBERS", JSON.stringify(state.members));
      }
      ...
      if (Array.isArray(data.deletedMembers)) {
        ...
        safeLocalStorage.setItem("CHURCH_DELETED_MEMBERS", JSON.stringify(state.deletedMembers));
      }
      if (data.adminPassword) {
        state.adminPassword = data.adminPassword;
        safeLocalStorage.setItem("CHURCH_ADMIN_PASS", state.adminPassword);
      }
      if (data.currentReceiptNo) {
        state.currentReceiptNo = data.currentReceiptNo;
        safeLocalStorage.setItem("CHURCH_RECEIPT_NO", state.currentReceiptNo);
      }
      if (data.currentVoucherNo) {
        state.currentVoucherNo = data.currentVoucherNo;
        safeLocalStorage.setItem("CHURCH_VOUCHER_NO", state.currentVoucherNo);
      }

      // Ensure CURRENT_DATA_VERSION is set so loadAllData preserves restored values
      safeLocalStorage.setItem("CHURCH_DATA_VERSION", "2026-08-04_V7_4_FORCE_REFRESH");
...
function checkTrialLicenseGuard() {
  ...
  const isActivated = safeLocalStorage.getItem("CHURCH_TRIAL_ACTIVATED") === "true";
  if (isActivated) return;

  let startTime = parseInt(safeLocalStorage.getItem("CHURCH_TRIAL_START_TIME"), 10);
  if (isNaN(startTime) || !startTime) {
    startTime = Date.now();
    safeLocalStorage.setItem("CHURCH_TRIAL_START_TIME", startTime.toString());
  }
...
function verifyTrialActivationKey() {
  ...
  if (key === "CHURCH-2026-ACTIVATE" || key === state.adminPassword || key === "admin123" || key === "church123") {
    safeLocalStorage.setItem("CHURCH_TRIAL_ACTIVATED", "true");
    const modal = document.getElementById("trialActivationModal");
    if (modal) {
