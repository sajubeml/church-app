...
function getAllAccountHeads(type = "ALL") {
  ...
  function addHead(rawCode, rawName, category, source, override = false) {
    ...
    // Budget Master heads take 100% HIGHEST priority and can NEVER be overwritten by Trial Balance, Cashbook, or Codes.json (unless Custom or Edited)
    if (existing && existing.source === "Budget Master" && source !== "Budget Master" && source !== "Custom" && source !== "Edited") {
      return;
    }
...
function saveClientSidePdfSilent(containerOrHtml, pdfFilename) {
  try {
    ...
    html2pdf().set(opt).from(wrappedHtml).output('blob').then(async function (pdfBlob) {
      // 1. Silent Local Download
      ...
      // 2. Silent Cloud Upload
      try {
        const token = window.SUPABASE_ACCESS_TOKEN;
        if (token) {
          const storageUrl = `${SUPABASE_URL}/storage/v1/object/receipts/${pdfFilename}`;
          ...
          if (uploadRes.ok) console.log(`[Cloud] PDF archived: ${pdfFilename}`);
        }
      } catch (e) {
        console.warn('[Cloud] PDF upload error:', e);
      }
    }).catch(function (err) {
...
function exportReceiptPdf() {
  ...
  // 1. Client-Side PDF Generation for Web App (Download + Cloud Upload)
  if (typeof window.html2pdf !== 'undefined') {
    ...
    window.html2pdf().set(opt).from(modalArea).output('blob').then(async function (pdfBlob) {
      // 1a. Silent Local Download
      const url = URL.createObjectURL(pdfBlob);
      const a = document.createElement("a");
      a.href = url;
      a.download = pdfName;
      ...
      // 1b. Silent Cloud Upload
      try {
        const token = window.SUPABASE_ACCESS_TOKEN;
        if (token) {
          const storageUrl = `${SUPABASE_URL}/storage/v1/object/receipts/${pdfName}`;
          const uploadRes = await fetch(storageUrl, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
          ...
          if (uploadRes.ok) console.log(`[Cloud] PDF archived: ${pdfName}`);
        }
      } catch (e) {
        console.warn('[Cloud] PDF upload error:', e);
      }
  ...
  // 2. Preserve existing logic: server archive and fallback
...
function saveAccountHeadChanges() {
  ...
  const updatedObj = { code: newCode, name: newTitle, category, type: category, source: "Custom" };
